...
func start_game():
	areaposx = get_window().get_size().x
	collision_shape_2d.position.x=areaposx/2
	areaposy = get_window().get_size().y
	collision_shape_2d.position.y=areaposy/2
	if areaposy > areaposx:
		collision_shape_2d.shape.size.x=areaposx
		collision_shape_2d.shape.size.y=areaposx
	else:
		collision_shape_2d.shape.size.x=areaposy
		collision_shape_2d.shape.size.y=areaposy
	tile_h = collision_shape_2d.shape.size.x/4
	var image = Image.load_from_file("res://img/Thousand yard lorax.png")
